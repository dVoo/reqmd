// Package cpp implements the C++ language plugin for reqmd-import:
//
// it parses a C++ source file via tree-sitter and emits a flat list of
// top-level symbols (functions, methods, types, globals, macros).
//
// # Plugin shape
//
// CppLanguage implements internal/lang.Language. Each call to Parse
...
//
//	model.SymbolFunction - free functions (`int foo(...) { ... }`) and
//	                       their namespace-scope prototypes
//	model.SymbolMethod   - class/struct member functions, both inline
//	                       definitions and in-class declarations; the
//	                       Receiver field carries the enclosing class
//	model.SymbolType     - class / struct / enum / union specifiers,
//	                       `using X = ...;` aliases, and typedef aliases
//	model.SymbolVar      - top-level variable declarations
//	model.SymbolConst    - object-like `#define FOO 100` macros
//
// The Package field defaults to the file's basename without extension
// (C++ has no package clause); the extractor may rewrite it from the
// on-disk module path.
package cpp
...
// Compile-time assertion: CppLanguage satisfies lang.Language.
var _ lang.Language = (*CppLanguage)(nil)

// Name returns the canonical language identifier.
func (l *CppLanguage) Name() string { return "cpp" }
...
func (l *CppLanguage) Extensions() []string {
	return []string{".cpp", ".cc", ".cxx", ".hpp", ".hh", ".hxx"}
}

// Grammar returns the tree-sitter-cpp grammar language.
func (l *CppLanguage) Grammar() *sitter.Language { return sitter.NewLanguage(tree_sitter_cpp.Language()) }
...
func (l *CppLanguage) Query() string {
...
func (l *CppLanguage) BindDoc(node *sitter.Node, src []byte) string {
	if node == nil {
		return ""
	}
...
func (l *CppLanguage) Parse(file string, src []byte) ([]model.Symbol, error) {
	...
	// C++ has no package clause; fall back to the file's basename
...
// declaration.
func symbolFromMatch(captures map[string]*sitter.Node, file, pkg string, src []byte, l *CppLanguage) (model.Symbol, bool) {
	switch {
	...
	case captures["var.decl"] != nil:
		...
		// as namespace-level ones; drop them so two same-named locals in
