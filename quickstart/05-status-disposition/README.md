...
explicitly state that no downstream coverage is expected. When an entire layer
expects no downstream coverage, declare it once per document instead of on
every requirement:
...
Every requirement in the document inherits the empty default; a requirement may
still override it (or the reverse — see [Step 2](../02-trace-your-spec/)).
